...
class TrajectoryPlanner:
	...
	def MPC(self):
		...
		for t in range(N):
			...
			constr = [ x[:,t+1] == model.A2*x[:,t] + model.B2*w[:,t],

					   # constarints on the inputs
	                   mp.u_min <= w[0,t], w[0,t] <= mp.u_max, 
	                   mp.u_min <= w[1,t], w[1,t] <= mp.u_max, 
	                   mp.u_min <= w[2,t], w[2,t] <= mp.u_max, 
	                   -np.pi <= w[3,t], w[3,t] <= np.pi]
	...
	def CalculateE5Static(self):
		...
		for i in range(mp.n_stat_obst):
			static_ref[:,i] 		= np.array((l_st_obs + int(stat_obst_pos[i][0]), 
										...
										l_st_obs - int(stat_obst_pos[i][1]), 
										l_st_obs - int(stat_obst_pos[i][2])))
			E5_static[22*i:22*(i+1)]= np.concatenate((np.add(mld.Big_M.reshape((6, 1)),static_ref[:,i].reshape((6, 1)) ),
	...
	def CalculateE5Dynam(self):
		...
		for i in range(mp.n_dynam_obst):
			dynamic_ref[:,i] 		= np.array((l_dy_obs + int(dynam_obst_pos[i][0]), 
										...
										l_dy_obs - int(dynam_obst_pos[i][1]), 
										l_dy_obs - int(dynam_obst_pos[i][2])))
			E5_dynam[22*i:22*(i+1)]= np.concatenate((np.add(mld.Big_M.reshape((6, 1)),dynamic_ref[:,i].reshape((6, 1)) ),
	...
	def SimulateQuad(self,x,u):
		...
		return model.A2*x + model.B2*u + z_int.reshape((9,1))
	...
	def run(self):
		...
		x_r.value = np.concatenate((mp.obst_centers[mp.endindx],np.array([0,0,0,0,np.pi/4,int(mp.obst_centers[mp.endindx,2])])))
		#print(x_r.value)
